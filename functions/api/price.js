// Fonction serveur Cloudflare Pages — récupère le prix d'un ETF via Yahoo Finance.
// Tourne côté serveur (pas dans le navigateur), donc PAS de blocage CORS.
//
// Appel depuis l'app :  /api/price?isin=FR0011550185
//                   ou  /api/price?ticker=ESE.PA
//
// Réponse JSON : { price: 26.5, currency: "EUR", symbol: "ESE.PA", name: "...", source: "Yahoo" }
//          ou   { error: "raison" }
//
// NOTE Cloudflare : ce fichier vit dans /functions/api/price.js et devient
// automatiquement accessible à l'adresse /api/price (Pages Functions).
// La syntaxe diffère de Netlify : on exporte onRequest(context) et on renvoie
// un objet Response standard, au lieu de { statusCode, body }.
...
// Comme tout le monde suit les mêmes grands ETF (World, S&P 500…), ça évite
// de rappeler Yahoo pour un prix déjà connu. Les cours Yahoo sont des clôtures
// quotidiennes : un cache de quelques heures ne fait perdre aucune fraîcheur.
...
// Petit utilitaire pour renvoyer du JSON avec les bons en-têtes (CORS + cache)
...
// Point d'entrée Cloudflare Pages Functions.
// context.request contient la requête ; les paramètres sont dans l'URL.
export async function onRequest(context) {
  try {
    ...
    if (!isin && !ticker) {
      return jsonResponse({ error: 'Aucun ISIN ni ticker fourni' });
    }
    ...
    if (wantHistory) {
      ...
      if (!symbol) {
        return jsonResponse({ error: 'Symbole introuvable pour l\'historique' });
      }
      const hist = await fetchYahooHistory(symbol);
      if (!hist) {
        return jsonResponse({ error: 'Historique insuffisant', symbol });
      }
